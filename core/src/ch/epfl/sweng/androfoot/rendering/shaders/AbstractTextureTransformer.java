...
public abstract class AbstractTextureTransformer implements TextureTransformer {
	 ...
	 * @param widthArg the width of the texture
	 * @param heightArg the height of the texture
	 */
	...
	/**
	 * Allow the subclasses to choose their implementation of {@link DrawToTexture}
	 * (GOF factory)
	...
	/**
	 * Callback to allow the subclasses to set the arguments on the shader (after shader.begin())
	 * @param shader the current shader
	 */
	protected abstract void setShaderArguments(ShaderProgram shader);

	/**
	 * Callback to allow the subclasses to configure the spriteBatch before it is used to render the texture
	 * @param batch the sprite batch the class has to configure
	 */
	protected abstract void configureSpriteBatch(SpriteBatch batch);
	
	/**
	 * Callback to allow the subclasses to do a setup on their shader (after compilation)
	 * @param shader the shader to configure
	 */
	protected abstract void setupShader(ShaderProgram shader);
